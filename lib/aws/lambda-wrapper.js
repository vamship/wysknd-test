...
class LambdaWrapper {
    ...
    constructor(handler, event, contextProps, configProps) {
        ...
        this._invokePromise = new Promise((resolve, reject) => {
            this._callback = _sinon.stub(wrapper, '_callback').callsFake((err, data) => {
                if(err) {
                    reject(err);
                    return;
                }
    ...
    _initExt(configProps, env) {
        ...
        // These methods are injected by the lambda wrapper

        // Special method to log extractable metrics
        logger.metrics = _sinon.spy();

        // Special method to log duration of specific events
        logger.timespan = _sinon.spy();
    ...
    testError(error, message) {
        this.callback.resetHistory();
        this.invoke().catch((ex) => {
            //Handle the rejection without errors. Assertion checks
